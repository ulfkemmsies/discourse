...
class UserApiKey < ActiveRecord::Base
  ...
  REVOKE_SELF_MATCHER = RouteMatcher.new(actions: "user_api_keys#revoke", methods: :post, params: [:id])
  ...
  def self.available_scopes
    @available_scopes ||= Set.new(UserApiKeyScopes::SCOPES.keys.map(&:to_s))
  end
  ...
  def is_revoke_self_request?(env)
    REVOKE_SELF_MATCHER.match?(env: env, allowed_param_values: { "id" => [nil, id.to_s] })
  end
end
